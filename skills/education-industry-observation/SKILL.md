...
4. 通过事实和来源门槛后，按 `event-analysis-template.md` 评分并填写研究卡；合并重复事项。
5. 按 `report-requirements.md` 补齐事项类型信息，归纳内部核心观点和本期行业小结。
6. 按 `report-template.md` 生成 JSON，再用生成器执行 `quality-check.md` 对应的自动门槛。
7. 生成 PPT、来源清单和质量报告，渲染全部页面检查溢出、黑屏、重叠、字体、来源和页码。
